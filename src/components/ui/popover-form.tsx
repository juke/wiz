...
export function PopoverForm({
  ...
  const openChildRef = useRef<HTMLDivElement>(null)

  useClickOutside(popoverRef, () => setOpen(false))

  useEffect(() => {
    if (open && popoverRef.current) {
      // Animate popover opening
  ...
  useEffect(() => {
    if (showSuccess && successRef.current && openChildRef.current) {
      // Animate to success state
      gsap.to(openChildRef.current, {
        y: 8,
        opacity: 0,
        filter: "blur(4px)",
        duration: 0.4,
    ...
    } else if (!showSuccess && openChildRef.current) {
      // Reset to open state
      gsap.set(openChildRef.current, {
        y: 0,
        opacity: 1,
        filter: "blur(0px)"
      })
  ...
  return (
    <div
      key={title}
      className="flex w-full items-center justify-center z-10"
    >
      {open && (
        <div
          ref={popoverRef}
        ...
        >
          <span
            aria-hidden
            className={`absolute left-4 top-[17px] text-sm text-muted-foreground ${showSuccess ? 'text-transparent' : ''}`}
          >
            {title}
          </span>
...
export function PopoverFormButton({
  ...
  useEffect(() => {
    if (contentRef.current && prevLoadingRef.current !== loading) {
  ...
  return (
    <button
      type="submit"
      className="ml-auto cursor-pointer flex h-6 w-26 items-center justify-center overflow-hidden rounded-md bg-gradient-to-b from-primary/90 to-primary px-3 text-xs font-semibold text-primary-foreground shadow-[0_0_1px_1px_rgba(255,255,255,0.08)_inset,0_1px_1.5px_0_rgba(0,0,0,0.32),0_0_0_0.5px_#1a94ff]"
    >
      <span
        ref={contentRef}
